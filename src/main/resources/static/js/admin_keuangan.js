// File: js/admin_keuangan.js - VERSION READ-ONLY untuk ADMIN
// Menggunakan logika yang sama dengan manajer_keuangan.js 
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Cek apakah user adalah ADMIN (bisa view-only)
    ...
    console.log('👑 Admin Mode: View-only financial data');
    ...
    // Setup search/filter untuk tabel responsive (read-only)
    setupTableSearchFilter();

    // Setup pagination untuk tabel
    setupPaginationControls();

    // Download button untuk Excel (masih bisa diakses)
...
async function loadFinancialData() {
    try {
        console.log('🔄 Loading financial data for ADMIN (read-only)...');
        ...
        // PROSES DATA TERINTEGRASI (sama seperti manajer)
...
// ========== FUNGSI INTEGRASI (SAMA DENGAN MANAGER) ==========

function processIntegratedData(keuanganData, transaksiData) {
    console.log('🔧 Processing integrated data untuk ADMIN...');
...
function updateFinancialUI(integratedData) {
    console.log('🎨 Updating UI with integrated data (Admin)...');
...
function updateFinancialCards(pemasukan, pengeluaran, laba, transaksiCount) {
    console.log('💳 Updating cards for Admin:', { pemasukan, pengeluaran, laba, transaksiCount });

    // Update card values dengan data REAL
    const cards = [
        { selector: '#card-income', value: formatCurrency(pemasukan) },
        { selector: '#card-expense', value: formatCurrency(pengeluaran) },
        { selector: '#card-profit', value: formatCurrency(laba), color: laba >= 0 ? '#10b759' : '#ff5252' },
        { selector: '#card-transactions', value: `${transaksiCount} Transaksi` }
    ];
...
// ========== FUNGSI TABEL RESPONSIVE (READ-ONLY) ==========
...
function displayIntegratedRecordsResponsive(integratedData) {
    ...
    updateExpensesTotal(totalPengeluaran, totalPemasukan);
...
function renderTableWithPagination() {
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="7" class="empty-state">
                    ...
                    <p style="font-size:16px;margin:8px 0">${currentSearch || currentFilter ? 'Tidak ada data yang sesuai' : 'Belum ada data keuangan'}</p>
                    <p style="color:var(--text-secondary);font-size:14px">
                        ${currentSearch || currentFilter ? 'Coba kata kunci atau filter lain' : 'Data akan muncul setelah ada transaksi atau pengeluaran'}
                    </p>
                </td>
            </tr>
        `;
    ...
    // Render rows (READ-ONLY - tanpa delete button)
    pageRecords.forEach((record, index) => {
        const rowIndex = startIndex + index + 1;
        const row = createTableRowReadOnly(record, rowIndex);
...
function createTableRowReadOnly(record, index) {
    ...
    // FIX: Escape single quotes dengan benar
    const safeDescription = record.description ?
        record.description.replace(/'/g, "&#39;").replace(/"/g, "&quot;") : '';

    // READ-ONLY: Hanya view button, tidak ada delete/edit
    row.innerHTML = `
        ...
        <td class="action-column">
            <div class="action-buttons-group">
                <button class="btn-icon btn-view" data-record-id="${record.id}" title="Lihat Detail (Admin View)">
    ...
    // Tambahkan event listener untuk view button
    ...
    return records.filter(record => {
        ...
        if (filter) {
            if (filter === 'PEMASUKAN') {
                ...
                matchesFilter = record.type === 'PENGELUARAN';
            } else {
...
function updateExpensesTotal(pengeluaran, pemasukan) {
    const totalExpenseElement = document.getElementById('total-expenses');
    const totalIncomeElement = document.getElementById('total-income');
    
    if (totalExpenseElement) {
        totalExpenseElement.textContent = formatCurrency(pengeluaran);
    }
    
    if (totalIncomeElement) {
        totalIncomeElement.textContent = formatCurrency(pemasukan || 0);
    }
}
...
function updateBreakdownSummary(integratedData) {
    ...
    const summaryHtml = `
        ...
        <tr style="border-bottom:1px solid var(--border-color)">
            ...
            <td style="padding:12px 8px;text-align:right;color:#666;font-size:12px">
                Semua pengeluaran (stok, operasional, dll)
            </td>
        </tr>
...
// ========== RECORD VIEW FUNCTIONS (READ-ONLY) ==========
...
function viewRecordDetail(recordId) {
    ...
    // Buat modal custom dengan styling read-only
        ...
        ">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <div>
                    ...
                    <p style="margin: 4px 0 0 0; color: #6b7280; font-size: 12px;">
                        🔒 Admin View Only - Tidak dapat diubah
                    </p>
                </div>
            ...
            <div style="background: #fef3c7; border: 1px solid #fde68a; border-radius: 8px; padding: 12px; margin-bottom: 16px;">
                <div style="display: flex; align-items: center; gap: 8px; color: #92400e;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                        <path d="M12 15V12m0-3h.01M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z" 
                              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    <span style="font-size: 13px; font-weight: 500;">
                        Mode View Only: Data ini hanya dapat dilihat, tidak dapat diubah atau dihapus.
                    </span>
                </div>
            </div>
...
async function fetchKeuanganData() {
    try {
        console.log('🔍 Fetching keuangan data (Admin)...');
...
async function fetchTransaksiData() {
    try {
        console.log('🔍 Fetching transaksi data (Admin)...');
...
// ========== UTILITY FUNCTIONS (SAMA DENGAN MANAGER) ==========
    ...
    setInterval(() => {
        ...
        if (lastUpdate) {
            ...
            if (now - updateTime < 30000) {
                console.log('🔄 Detected finance update (Admin), refreshing...');
                loadFinancialData();
...
window.addEventListener('message', (event) => {
    if (event.data.type === 'FINANCE_UPDATE') {
        console.log('📩 Received finance update broadcast (Admin)');
        loadFinancialData();
    }
});
...
window.addEventListener('storage', (event) => {
    if (event.key === 'finance_needs_refresh') {
        console.log('💾 Storage event detected: finance needs refresh (Admin)');
        loadFinancialData();
    }
});